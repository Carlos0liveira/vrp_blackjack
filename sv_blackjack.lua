...
function giveChips(source,amount)
    local source = source
    local user_id = vRP.getUserId(source)
    if vRP.giveInventoryItem(user_id,"ficha",amount) then
       return true
    end
end
...
AddEventHandler("Blackjack:requestSitAtBlackjackTable", function(chairId)
    ...
    if source ~= nil then
        ...
        TriggerClientEvent("blackjack:notify",source,"~r~Este lugar já está ocupado")
            ...
            if tonumber(betAmount) then
                ...
                if betAmount > 0 then
                    if tryTakeChips(source,betAmount) then
                        ...
                        TriggerClientEvent("blackjack:notify",source,"~g~Valor da aposta: " .. tostring(betAmount) .. " fichas.")
                    else 
                        TriggerClientEvent("blackjack:notify",source,"~r~Voce nao tem fichas suficientes!")
                    end
                end
            end
        end
    else
        TriggerClientEvent("blackjack:notify",source,"~r~Erro ao apostar!")
    end
end)
