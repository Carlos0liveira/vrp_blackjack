...
Citizen.CreateThread(function()
    ...
    for i=0,#cfg.blackjackTables,1 do
        ...
        TeleportPlayerNoOffset(dealerEntity, cfg.blackjackTables[i].dealerPos.x,cfg.blackjackTables[i].dealerPos.y,cfg.blackjackTables[i].dealerPos.z, 0,0,1)
            ...
            if closestChair ~= nil and closestChairDist < 2 then
                if not timeoutHowToBlackjack then
                    if blackjackTableData[closestChair] == false then 
                        drawNativeNotification("Pressione ~INPUT_PICKUP~ para jogar o blackjack")
                    else 
                        drawNativeNotification("Este assento está ocupado.")
                    end
...
Citizen.CreateThread(function()
    while true do 
        ...
        if drawCurrentHand then 
            ...
            AddTextComponentString("Sua mão é ["..tostring(currentHand).."], a mão do deler é ["..tostring(dealersHand).."]")
    ...
    while true do 
        if waitingForBetState then
            if IsDisabledControlJustPressed(0, 22) then --Custom Bet [space]
                local tmpInput = getGenericTextInput("Valor da aposta")
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('cmgblackjack', 'instructions')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            RageUI.FakeButton("test", "O objetivo do Blackjack é vencer a mão do Dealer sem ultrapassar 21                                                          Este jogo usa quatro baralhos de 52 cartas, que são embaralhados no início de cada mão.", { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if (Hovered) then
...
AddEventHandler("Blackjack:beginBetsBlackjack",function(gameID,tableId)
    ...
    ClearHelp(true)
    drawNativeNotification("Faça sua aposta")
    ...
    dealersHand = 0
    currentHand = 0
    TeleportPlayerNoOffset(dealerPed, cfg.blackjackTables[tableId].dealerPos.x,cfg.blackjackTables[tableId].dealerPos.y,cfg.blackjackTables[tableId].dealerPos.z, 0,0,1)
    ...
    Citizen.CreateThread(function()
        ...
        if not bettedThisRound then
            ...
            drawNativeNotification("Nenhuma aposta feita, rodada pulada")
        end
    end)
end)
        ...
        if closestChair == chairId then
            ...
            Citizen.CreateThread(function()
                ...
                if not standOrHitThisRound and sittingAtBlackjackTable then
                    ...
                    TriggerServerEvent("Blackjack:standBlackjack",globalGameId,globalNextCardCount)
                    declineCard()
                    drawNativeNotification("Tempo esgotado para pedir carta ou desistir")
                end
            end)
        else 
...
function goToBlackjackSeat(blackjackSeatID)
    ...
    drawNativeNotification("Aguardando o próximo jogo")
    blackjackAnimsToLoad = {
      "anim_casino_b@amb@casino@games@blackjack@dealer",
      "anim_casino_b@amb@casino@games@shared@dealer@",
      "anim_casino_b@amb@casino@games@blackjack@player",  
...
function startSingleDealerDealing(dealerPed,gameId,cardData,nextCardCount,gotCurrentHand,chairId,tableId)
    ...
    if DoesEntityExist(dealerPed) then
        ...
        TeleportPlayerNoOffset(nextCardObj, cardPos.x, cardPos.y, cardPos.z, 0, 0, 1)
        Wait(400)
    else 
...
function flipDealerCard(dealerPed,gotCurrentHand,tableId,gameId)
    ...
    local soundCardString = "MINIGAME_BJACK_DEALER_" .. tostring(gotCurrentHand)
    PlayAmbientSpeech1(dealerPed,soundCardString,"SPEECH_PARAMS_FORCE_NORMAL_CLEAR",1)
    TeleportPlayerNoOffset(cardObj, cardX,cardY,cardZ)
end
...
AddEventHandler("Blackjack:blackjackLose",function(tableId)
    if closeToCasino then
        ...
        angryILost()
        canExitBlackjack = true
        drawNativeNotification("~r~Você Perdeu!")
        drawCurrentHand = false
        currentHand = 0
        dealersHand = 0
    end
...
AddEventHandler("Blackjack:blackjackPush",function(tableId)
    if closeToCasino then
        ...
        annoyedIPushed()
        canExitBlackjack = true
        drawNativeNotification("~b~Rodada Empatada")
        drawCurrentHand = false
        currentHand = 0
        dealersHand = 0
    end
...
AddEventHandler("Blackjack:blackjackWin",function(tableId)
    if closeToCasino then
        ...
        happyIWon()
        canExitBlackjack = true
        drawNativeNotification("~g~Você Ganhou!!!")
        drawCurrentHand = false
        currentHand = 0
        dealersHand = 0
    end
...
function betChipsForNextHand(chipsAmount,chipsProp,something,chairID,someBool,zOffset)
    ...
    TeleportPlayerNoOffset(chipsObj, chipsOffset.x, chipsOffset.y, chipsOffset.z+zOffset, 0, 0, 1)
...
function getNewCardFromMachine(nextCard,chairId,gameId)
    ...
    SetPedVisible(nextCardObj,false)
    SetModelAsNoLongerNeeded(nextCardHash)
    local cardObjectOffsetRotation = blackjack_func_398(blackjack_func_368(chairId))
    TeleportPlayerNoOffset(nextCardObj, cardObjectOffset.x, cardObjectOffset.y, cardObjectOffset.z, 0, 0, 1)
    --vVar8 =  vector3(0.0, 0.0, getTableHeading(blackjack_func_368(chairId)))
...
function getGenericTextInput(type)
	...
	DisplayOnscreenKeyboard(1, "FMMC_MPM_NA", "" .. tostring(type) .. " message", "", "", "", "", 30)
